...
describe('AudioLevelIndicator', () => {
    it('renders 10 bars', () => {
        const { container } = render(_jsx(AudioLevelIndicator, { level: 0, isActive: false }));
    ...
    it('shows inactive bars when not active', () => {
        const { container } = render(_jsx(AudioLevelIndicator, { level: 50, isActive: false }));
        const flexContainer = container.querySelector('.flex.items-center');
    ...
    it('shows active bars based on level when active', () => {
        const { container } = render(_jsx(AudioLevelIndicator, { level: 50, isActive: true }));
        const flexContainer = container.querySelector('.flex.items-center');
        const bars = Array.from(flexContainer?.children || []);
    ...
    it('shows different colors for high levels', () => {
        const { container } = render(_jsx(AudioLevelIndicator, { level: 100, isActive: true }));
        const flexContainer = container.querySelector('.flex.items-center');
        const bars = Array.from(flexContainer?.children || []);
    ...
    it('sets opacity based on active state', () => {
        const { container } = render(_jsx(AudioLevelIndicator, { level: 30, isActive: true }));
        const flexContainer = container.querySelector('.flex.items-center');
        const bars = Array.from(flexContainer?.children || []);
        // First 3 bars should be active (opacity 1)
        for (let i = 0; i < 3; i++) {
            expect(bars[i].style.opacity).toBe('1');
        }
        // Remaining bars should be inactive (opacity 0.3)
        for (let i = 3; i < 10; i++) {
            expect(bars[i].style.opacity).toBe('0.3');
        }
    });
});
